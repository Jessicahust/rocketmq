 ...
 * RocketMQ却为Producer和Consumer分别设计了不同的存储结构，Producer对应CommitLog, Consumer对应ConsumeQueue
 * 只要消息到了CommitLog，发送的消息也就不会丢。只要消息不丢，那就有了“充足的回旋余地”，用一个后台线程慢慢同步到ConsumeQueue，再由Consumer消费。
 */
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        try {
            ...
            if (null == mappedFile || mappedFile.isFull()) {
                //如果被锁的最后一个映射文件不允许写入，那就只能获取映射文件的最后一个文件了,创建一个mappedFile
        ...
        //消息刷盘，即持久化到文件。上面插入消息实际未存储到硬盘。此处，根据不同的刷盘策略，执行会有不同
        ...
        // Asynchronous flush
            ...
            if (!this.defaultMessageStore.getMessageStoreConfig().isTransientStorePoolEnable()) {
                flushCommitLogService.wakeup(); // 异步刷盘 && 关闭内存字节缓冲区
            } else {
                commitLogService.wakeup();  //异步刷盘 && 开启内存字节缓冲区
            }
        }
