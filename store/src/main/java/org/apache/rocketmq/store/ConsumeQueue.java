...
/**
 * ConsumeQueue是消息的逻辑队列，相当于字典的的目录，用来指定消息在真正的物理文件上的存储位置。每个topic下的每个queue都有一个对应的ConsumeQueue文件
 */
public class ConsumeQueue {
    ...
    /**
     * 每个 ConsumeQueue 的 mappedFiles 集合中,从倒数第三个文件开始恢复(为什 么只恢复倒数三个文件,也许只是个经验值吧)
     *  consumequeue 的存储单 元是 20 字节的定长数据
     */
    public void recover() {
        ...
        if (!mappedFiles.isEmpty()) {
            ...
            /**
             * processOffset 是有效数据的偏移量,获取这个值的作用什么?
             * (1) processOffset 后面的数据属于脏数据,后面的文件要删除掉
             * (2) 设置processOffset 所 在 文 件 MappedFile 的 wrotePosition 和
             * committedPosition 值, 值为 processOffset%mappedFileSize
             */
            processOffset += mappedFileOffset;
